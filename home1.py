...
def copy_row(source_filename, target_filename):
    row_number = int(input("Введите номер строки для копирования: "))
    source_data = read_file(source_filename)

    if 0 < row_number <= len(source_data):
        row_to_copy = source_data[row_number - 1]
        write_file(target_filename, [row_to_copy["Имя"], row_to_copy["Фамилия"], row_to_copy["Телефон"]])
        print("Данные скопированы успешно.")
    else:
        print("Неверный номер строки.")

filename = 'phone.csv'
target_filename = 'copy_phone.csv'
...
def main():
    while True:
        ...
        if command == "q":
            ...
            copy_row(filename, target_filename)
